window.view = {
	...
	displayFunctionForRightAngleTriangle: function() {
		document.getElementById('functionRightAngleTriangle').innerHTML += 'float area_rtriangle (float a,float b)<br>{<br> &emsp; float area = 1/2*a*b;<br> &emsp; return area;<br>}'
	},
	...
	area_rtriangle: function( param, functionCall ) {
	...
	calculateArea: function() {
		...
		for ( i = 0 ; i < elements.length ; i ++ ) {
			...
			else if ( parameter.length === 2 && functionCall === 'area_rtriangle()' && isNaN(parameter[0]) === false && isNaN(parameter[1]) === false )
				this.area_rtriangle(parameter, parsedValue)
			else
		 	alert( 'Incorrect function call at line ' + String( i + 1 ) )
		}	
